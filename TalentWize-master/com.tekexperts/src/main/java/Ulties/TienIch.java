...
public class TienIch {
	 ...
	 * 
	 */
	public static String getAbsoluteFilePathFromResource(String fileName) {
		resource = TienIch.class.getProtectionDomain().getCodeSource().getLocation();

		tr2 = resource.toString().replace("file:/", "").replace("%20", " ").replace("/", "\\").replace("target", "@");
		int l = tr2.indexOf("@");
		spath = tr2.substring(0, l).replace("@", "") + "src\\test\\resources\\" + fileName;
	...
	public static String getResources(String filename) {
		URL resource = WebActions.class.getProtectionDomain().getCodeSource().getLocation();
	 ...
	 * 
	 * @param filename
	 * @return source path folder path of resources needed
	 */
	public static String getAbsolutePathFromResource(String nameOfFile)
	{
		File f = new File("src/test/resources/"+nameOfFile);
		if (f.exists())
		{
		...
		{
			return "There is NOT such file name ! " ; 
		}
	}
